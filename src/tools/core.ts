...
import { z } from "zod";
import { searchIdentities, searchIdentitiesWithConnection } from "./auth.js";
...
/**
 * Configure core tools with organization context awareness
 */
function configureCoreToolsWithContext(server: McpServer, orgManager: OrganizationManager) {
  server.tool(
    CORE_TOOLS.list_project_teams,
    "Retrieve a list of teams for the specified Azure DevOps project.",
    {
  ...
  server.tool(
    ...
    withOrganizationContext(
      ...
      async ({ searchFilter }) => {
        ...
        // Use the context-aware connection for identity search
        const identities = await searchIdentitiesWithConnection(searchFilter, connection);
...
export { CORE_TOOLS, configureCoreTools, configureCoreToolsWithContext };
